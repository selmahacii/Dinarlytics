...
const ChatbotLIA: React.FC = () => {
  ...
  const generateLiaResponse = async (userMessage: string): Promise<Message> => {
    ...
    // Générer le plan depuis l'analyse (commande / intention)
    const wantsPlanFromAnalysis =
      message.includes('/plan-analyse') ||
      (message.includes('générer') && message.includes('plan') && message.includes('analyse')) ||
      (message.includes('generer') && message.includes('plan') && message.includes('analyse'));
    if (wantsPlanFromAnalysis) {
      const result = seedPlanFromAnalysis();
      if (!result.ok) {
        return {
          id: Date.now().toString(),
          type: 'lia',
          content: `Impossible de générer le plan: ${result.reason}.`,
          timestamp: new Date(),
      ...
      return {
        id: Date.now().toString(),
        type: 'lia',
        content: `Plan d’actions généré depuis l’analyse (${(result as any).added ?? 0} nouvelles actions). Ouvrez le panneau pour les consulter.`,
        timestamp: new Date(),
        suggestions: ['Voir plan d’actions', 'Analyse financière', 'Exporter la conversation']
      };
    }
