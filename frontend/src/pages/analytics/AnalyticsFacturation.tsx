...
const AnalyticsFacturation: React.FC = () => {
  ...
  // Helper simple pour convertir les nombres en lettres (Français) - Version simplifiée pour la démo
  const numberToWords = (num: number) => {
    ...
    // Pour une démo pro, on va juste renvoyer une chaîne formatée réaliste
    return `${num.toLocaleString('fr-FR')} Dinars Algériens et zéro centimes`;
  };
  ...
  const [newInvoice, setNewInvoice] = useState({
    ...
    items: [{ desc: '', qty: 1, pu: 0 }],
    paymentMode: 'virement',
    notes: ''
  });

  const addItem = () => {
    setNewInvoice({ ...newInvoice, items: [...newInvoice.items, { desc: '', qty: 1, pu: 0 }] });
  };
  ...
  const invoices = useMemo(() => [
    {
      ...
      paymentMode: 'virement',
      items: [
        { desc: 'Ciment Portland CPJ 42.5 (Sac 50kg)', qty: 200, pu: 850 },
        { desc: 'Rond à béton 12mm (Tonne)', qty: 5, pu: 115000 },
        { desc: 'Briques creuses 8 trous', qty: 5000, pu: 25 }
      ],
      tvaRate: 19,
      statut: 'payee',
      secteur: 'btp',
    ...
    },
    {
      ...
      paymentMode: 'cheque',
      items: [
        { desc: 'Installation Fibre Optique (Forfait)', qty: 1, pu: 350000 },
        { desc: 'Configuration Routeurs Cisco', qty: 2, pu: 50000 }
      ],
      tvaRate: 19,
      statut: 'payee',
      secteur: 'services',
    ...
    },
    {
      ...
      paymentMode: 'virement',
      items: [
        { desc: 'Main d\'œuvre technique (Heures)', qty: 120, pu: 4500 },
        { desc: 'Maintenance préventive groupe électrogène', qty: 2, pu: 155000 },
        { desc: 'Kit de rechange filtration Heavy Duty', qty: 10, pu: 225000 }
      ],
      tvaRate: 19,
      statut: 'en_retard',
      secteur: 'industrie',
  ...
  return (
    <div className="space-y-8 p-6 bg-slate-50 dark:bg-slate-900 min-h-screen">
      {/* Header Premium */}
      <div className="bg-white dark:bg-slate-800 rounded-3xl p-8 shadow-sm border border-slate-200 dark:border-slate-700 relative overflow-hidden group">
        ...
        <div className="relative z-10 flex flex-col md:flex-row justify-between items-start md:items-center gap-6">
          <div>
            <h1 className="text-3xl font-black text-slate-900 dark:text-white flex items-center">
              <DocumentTextIcon className="h-10 w-10 text-emerald-600 mr-4 p-2 bg-emerald-50 rounded-xl" />
              Facturation Client
            </h1>
            <p className="text-slate-500 dark:text-slate-400 mt-2 font-medium">Gestion et pilotage du poste clients algérien</p>
          </div>
          <div className="flex gap-3">
            <button
              onClick={() => setIsCreateModalOpen(true)}
              className="px-6 py-2.5 bg-emerald-600 text-white rounded-xl font-bold hover:bg-emerald-700 transition-all shadow-lg shadow-emerald-600/20 flex items-center"
            >
              <PlusIcon className="h-5 w-5 mr-2" /> Nouvelle Facture
            </button>
            <button className="p-2.5 bg-slate-100 dark:bg-slate-700 text-slate-600 dark:text-slate-300 rounded-xl hover:bg-slate-200 transition-all">
      ...
      <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-6">
        <div className="bg-white dark:bg-slate-800 p-6 rounded-3xl shadow-sm border border-slate-200 dark:border-slate-700">
          <div className="flex items-center justify-between mb-4">
            <div className="p-3 bg-blue-50 dark:bg-blue-900/20 rounded-2xl text-blue-600"><CurrencyDollarIcon className="h-6 w-6" /></div>
      ...
      <div className="grid grid-cols-1 lg:grid-cols-3 gap-8">
        ...
        <div className="bg-white dark:bg-slate-800 rounded-3xl p-8 shadow-sm border border-slate-200 dark:border-slate-700">
          ...
          <div className="mt-8 p-4 bg-emerald-50 dark:bg-emerald-900/10 rounded-2xl border border-emerald-100 dark:border-emerald-800/30">
            <div className="flex items-center text-emerald-700 dark:text-emerald-400 text-sm font-bold">
              <SparklesIcon className="h-5 w-5 mr-2" /> Conseil IA
            </div>
            <p className="text-xs text-emerald-600 dark:text-emerald-500/80 mt-1 leading-relaxed">
                ...
                <tr key={inv.id} className="hover:bg-slate-50/80 dark:hover:bg-slate-900/30 transition-colors group">
                  ...
                  <td className="px-8 py-5">
                    <div className="flex justify-center gap-2 opacity-60 group-hover:opacity-100 transition-opacity">
                      <button className="p-2.5 bg-white dark:bg-slate-700 border border-slate-200 dark:border-slate-600 rounded-xl text-slate-500 hover:text-emerald-500 hover:border-emerald-500 shadow-sm transition-all" title="Voir PDF">
                        <PrinterIcon className="h-4 w-4" />
                      </button>
                      <button
                        onClick={() => openDetails(inv)}
                        className="p-2.5 bg-white dark:bg-slate-700 border border-slate-200 dark:border-slate-600 rounded-xl text-slate-500 hover:text-blue-500 hover:border-blue-500 shadow-sm transition-all"
                        title="Détails"
                      >
      ...
      {isCreateModalOpen && (
        <div className="fixed inset-0 bg-slate-900/60 backdrop-blur-md flex items-center justify-center p-4 z-50 animate-fade-in">
          <div className="bg-white dark:bg-slate-800 rounded-[2.5rem] shadow-2xl w-full max-w-4xl max-h-[90vh] overflow-y-auto border border-white/20 relative">
            <button
            ...
            <div className="p-10">
              ...
              <div className="mb-8">
                <div className="flex justify-between items-center mb-4">
                  ...
                  <button
                    onClick={addItem}
                    className="px-4 py-2 bg-emerald-50 text-emerald-600 rounded-xl text-xs font-black hover:bg-emerald-100 transition-all flex items-center"
                  >
                ...
                <div className="space-y-3">
                  {newInvoice.items.map((item, idx) => (
                    <div key={idx} className="grid grid-cols-12 gap-4 items-center bg-white dark:bg-slate-800 p-4 rounded-2xl border border-slate-100 dark:border-slate-700 shadow-sm group">
                      <div className="col-span-6">
                        <input
                          type="text"
                          placeholder="Désignation de l'article..."
                          className="w-full bg-slate-50 dark:bg-slate-900/50 border-none rounded-xl px-4 py-3 text-sm font-bold outline-none focus:ring-2 focus:ring-emerald-500"
                          value={item.desc}
                          onChange={(e) => {
                      ...
                      <div className="col-span-2">
                        <input
                          type="number"
                          placeholder="Qté"
                          className="w-full bg-slate-50 dark:bg-slate-900/50 border-none rounded-xl px-4 py-3 text-sm font-bold outline-none focus:ring-2 focus:ring-emerald-500 text-center"
                          value={item.qty}
                          onChange={(e) => {
                            const items = [...newInvoice.items];
                      ...
                      <div className="col-span-3">
                        <input
                          type="number"
                          placeholder="P.U HT"
                          className="w-full bg-slate-50 dark:bg-slate-900/50 border-none rounded-xl px-4 py-3 text-sm font-black outline-none focus:ring-2 focus:ring-emerald-500 text-right font-mono"
                          value={item.pu}
                          onChange={(e) => {
                            const items = [...newInvoice.items];
                            items[idx].pu = parseInt(e.target.value) || 0;
                            setNewInvoice({ ...newInvoice, items });
                          }}
                        />
                      </div>
              ...
              <div className="grid grid-cols-1 md:grid-cols-3 gap-8 mb-8">
                ...
                <div className="bg-emerald-600 rounded-3xl p-8 text-white shadow-2xl shadow-emerald-600/30 flex flex-col justify-between">
                  <div>
                    <span className="text-[10px] font-black uppercase tracking-widest text-white/60">Total Global TTC</span>
                  ...
                  <button
                    onClick={() => setIsCreateModalOpen(false)}
                    className="w-full py-4 bg-white text-emerald-700 rounded-2xl font-black text-xs uppercase tracking-widest hover:bg-emerald-50 transition-all mt-6"
                  >
                    Valider l'Émission
                  </button>
                </div>
              ...
              <div className="flex flex-col md:flex-row justify-between gap-12">
                ...
                <div className="flex-1">
                  <div className="flex items-center mb-6">
                    <div className="bg-emerald-600 p-2 rounded-lg mr-3 shadow-lg shadow-emerald-600/20">
                ...
                <div className="flex-1 md:text-right border-l md:border-l-0 md:border-r border-slate-100 dark:border-slate-800 px-0 md:px-12 order-first md:order-none">
                  ...
                  <p className="text-sm font-black text-emerald-600 uppercase">Facture de Vente Définitive</p>
            ...
            <div className="p-12">
              ...
              <div className="grid grid-cols-1 lg:grid-cols-2 gap-12 mt-12 items-start">
                ...
                <div className="space-y-3">
                  ...
                  <div className="flex justify-between items-center py-6 bg-emerald-600 text-white rounded-[1.5rem] px-8 shadow-xl shadow-emerald-600/20">
