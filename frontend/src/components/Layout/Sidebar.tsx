...
const Sidebar: React.FC = () => {
  ...
  const menuItems: SidebarItem[] = ([
    ...
    has('lia-access') && {
      ...
      subItems: [
        has('lia-chatbot') && { path: '/lia/chatbot', icon: SparklesIcon, label: 'Chatbot LIA' },
        has('lia-analyses') && { path: '/lia/analyses', icon: ChartBarIcon, label: 'Analyses LIA' },
        has('lia-train') && { path: '/entrainement-modele-ia', icon: CpuChipIcon, label: 'Entraînement du Modèle' }
      ].filter(Boolean)
    },
    // Rapports & Analytics avec sous-liens
    has('rapports-basic') && {
      type: 'submenu',
      icon: ChartBarIcon,
      ...
      subItems: [
        has('rapports-ventes') && { path: '/rapports/ventes-clients', icon: ChartBarIcon, label: 'Ventes & Clients' },
        has('rapports-achats') && { path: '/rapports/achats-fournisseurs', icon: ShoppingCartIcon, label: 'Achats & Fournisseurs' },
        has('rapports-stocks') && { path: '/rapports/stocks-produits', icon: CubeIcon, label: 'Stocks & Produits' },
        has('rapports-tresorerie') && { path: '/rapports/tresorerie-banque', icon: BanknotesIcon, label: 'Trésorerie & Banque' },
        has('rapports-comptabilite') && { path: '/rapports/comptabilite-resultats', icon: CalculatorIcon, label: 'Comptabilité & Résultats' },
        has('rapports-fiscalite') && { path: '/rapports/fiscalite-declarations', icon: ScaleIcon, label: 'Fiscalité & Déclarations' },
        has('rapports-personnalises') && { path: '/rapports/personnalises-comparatifs', icon: DocumentChartBarIcon, label: 'Personnalisés & Comparatifs' }
      ].filter(Boolean)
    },
    // Tableau de bord
    has('dashboard-access') && {
      ...
      subItems: [
        has('dashboard-overview') && { path: '/dashboard', icon: HomeIcon, label: "Vue d'ensemble" },
        has('dashboard-charts') && { path: '/dashboard/graphiques', icon: ChartPieIcon, label: 'Graphiques interactifs' },
        has('dashboard-alerts') && { path: '/dashboard/alertes', icon: ExclamationTriangleIcon, label: 'Alertes intelligentes' },
        has('dashboard-calendar') && { path: '/dashboard/calendrier', icon: CalendarIcon, label: 'Calendrier & Rappels' }
      ].filter(Boolean)
    },
    // Gestion commerciale
    has('clients-manage') && {
      ...
      subItems: [
        has('clients-manage') && { path: '/clients', icon: UsersIcon, label: 'Clients & Ventes', subLabel: 'Gestion clientèle et ventes' },
        has('fournisseurs-manage') && { path: '/fournisseurs', icon: TruckIcon, label: 'Fournisseurs & Achats', subLabel: 'Gestion fournisseurs' },
        has('articles-manage') && { path: '/articles', icon: CubeIcon, label: 'Articles & Inventaire', subLabel: 'Catalogue et stock' },
        has('facturation-read') && { path: '/factures-vente', icon: DocumentTextIcon, label: 'Facturation & Paiements', subLabel: 'Facturation et encaissements' }
      ].filter(Boolean)
    },
    // Comptabilité
    has('comptabilite-read') && {
      ...
      subItems: [
        has('comptabilite-read') && {
          ...
          subItems: [
            has('comptabilite-read') && { path: '/comptabilite/journaux', icon: ClipboardDocumentListIcon, label: 'Journaux & Écritures' },
            has('comptabilite-read') && { path: '/comptabilite/etats', icon: DocumentTextIcon, label: 'États & Rapports' }
          ].filter(Boolean)
        },
        has('rapports-comptabilite') && { path: '/rapports/comptabilite-resultats', icon: ChartBarIcon, label: 'Comptabilité & Résultats' },
        has('template-document') && { path: '/template-document', icon: DocumentTextIcon, label: 'Templates Documents' },
        has('consolidation') && { path: '/consolidation', icon: CalculatorIcon, label: 'Consolidation' },
        has('gestion-entreprise') && { path: '/gestion-entreprise', icon: BuildingOfficeIcon, label: 'Gestion Entreprises' },
        has('admin-users') && { path: '/gestion-utilisateurs-acces', icon: UserGroupIcon, label: 'Utilisateurs & Accès' }
      ].filter(Boolean)
    },
    // Paramètres
    has('admin-settings') && { path: '/parametres', icon: CogIcon, label: 'Paramètres' }
  ] as Array<SidebarItem | false>).filter(Boolean) as SidebarItem[];

  return (
    <div className={`fixed left-0 top-0 h-screen bg-white border-r border-slate-200 text-slate-800 overflow-y-auto z-10 shadow-lg transition-all duration-300 ${
          ...
          isActive = location.pathname.startsWith('/rapports');
            ...
            return (
              <div key={index}>
                <button
                  ...
                  className={`w-full flex items-center ${isSidebarCollapsed ? 'justify-center' : 'justify-between'} px-4 py-3 rounded-lg transition-colors ${
                    isActive
            ...
            return (
              <Link
                key={item.path}
                to={item.path}
                className={`flex items-center ${isSidebarCollapsed ? 'justify-center' : ''} px-4 py-3 rounded-lg transition-colors ${
                  isActive
                    ? 'bg-slate-100 text-slate-800 border border-slate-300'
                    : 'text-slate-600 hover:bg-slate-50 hover:text-slate-800'
                }`}
